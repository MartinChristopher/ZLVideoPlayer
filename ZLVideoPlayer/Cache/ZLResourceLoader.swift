...
// AVPlayer播放器下载代理
class ZLResourceLoader: NSObject {
    // 加载队列
    ...
    deinit {
        // 取消并保存数据
        let url = originalURL
        // 防止锁逻辑卡到主线程，cancel放到子线程去做
        DispatchQueue.global().async {
            ZLDownloadManager.shared.cancelDownload(url: url)
        }
    }
    // 填充数据
    @discardableResult
...
extension ZLResourceLoader: AVAssetResourceLoaderDelegate {
    ...
    public func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForLoadingOfRequestedResource loadingRequest: AVAssetResourceLoadingRequest) -> Bool {
        ...
        ZLDownloadManager.shared.startDownload(url: originalURL, loadingRequest: loadingRequest, progress: { [weak self] (loadingRequest, task) in
            if nil == self {
                return
            }
    ...
    public func resourceLoader(_ resourceLoader: AVAssetResourceLoader, didCancel loadingRequest: AVAssetResourceLoadingRequest) {
        ...
        ZLDownloadManager.shared.cancelDownloadCallback(url: originalURL, loadingRequest: loadingRequest)
    }
